...
t4 <- tmp[18:22, ]
t5 <- tmp[23:25, ]
knitr::kable(list(t1,t2,t3,t4, t5), booktabs = T, valign = 't',row.names = F,
...
p1 <- order.df %>%
  ...
  theme(legend.title = element_blank(),
        ...
        legend.background = element_rect(fill = "lightgrey", color="white"))
```
...
Because the contrasts are orthogonal, we can simply pluck the estimates of the regression coefficients from table 3 and divide by two:
...
```{r model-reduced, fig.cap="Some caption about residuals"}
...
Perhaps there exists a transformation on $Y$ that can accomplish a few things: (i) simplify the model by only including main-effects, (ii) make residuals more normally distributed, and (iii) improve the overall fit and interpretability of the model. We will experiment with a Box-Cox transform on the response, $Y$.
...
model.BC <- lm(Y ~ ., data = output$df.transformed)
# anova(model.BC)
...
p3 <- wiscR::qqplot(model.BC$residuals) + theme_minimal() +
  ...
  ggtitle("QQ Normal of residuals \n after Box-Cox")
...
p4 <- effects.BC.df %>%
  ggplot(aes(y = Effect, x = abs(Estimate))) +
  geom_col() +
  geom_vline(aes(xintercept = lenth.BC$critical,  color = "Lenth's Critical")) +
  geom_vline(aes(xintercept = dong.BC$critical,  color = "Dong's Critical")) +
  theme_minimal() +
  xlab(TeX("$| \\hat{\\theta} |$")) +
  ylab("") +
...
View this as a pilot to a larger follow-up experiment. Data cannot be incorporated into main data set. Why?
